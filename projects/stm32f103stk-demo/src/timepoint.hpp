...
protected:
  ...
  typedef decltype(resources::systick::GetCounter()) counter_type;
...
public:
  ...
  static constexpr auto counter_base = resources::systick::reload_value;
  static constexpr auto ps_per_tick  = resources::systick::ps_per_tick;

  void set() volatile {
    systick = time::get_systick();
    counter = resources::systick::GetCounter();
  }
  ...
  void add_ns(unsigned int ns) {
    counter_type counter_ticks = (ns * 1000) / resources::systick::ps_per_tick + 1; // always rounded up
  ...
  unsigned int get_ns(void) {
    ...
    t = t * 1000 / resources::systick::ps_per_tick;
    return t;
  };
