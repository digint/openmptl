...
public:  
  ...
  using USARTx = Core::USART<usart_no>;

  typedef GpioOutput< 'A', 2,  cGpio::OutputConfig::alt_push_pull > gpio_tx;
  typedef GpioInput < 'A', 3,  cGpio::InputConfig::floating >       gpio_rx;

  typedef Irq<IrqNumber::Interrupt::USART2>   GlobalIrq; /**< USART2 global Interrupt */


  
  static_assert(usart_no != 1, "usart 1 is not yet supported, sorry...");
  static constexpr uint32_t apb1enr = ( // usart_no == 1 ? Core::RCC::APB2ENR::USART1EN::value : 
                                       usart_no == 2 ? Core::RCC::APB1ENR::USART2EN::value : 
                                       usart_no == 3 ? Core::RCC::APB1ENR::USART3EN::value : 
                                       0);
